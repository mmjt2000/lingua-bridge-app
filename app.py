...
from sheets_db import get_session_by_num
from sheets_db import update_session
from sheets_db import get_stats
from sheets_db import authenticate
from sheets_db import save_submission
from sheets_db import get_student_submissions
from sheets_db import get_pending_submissions
from sheets_db import get_all_submissions_with_feedback
from sheets_db import save_feedback
...
<style>
    ...
    section[data-testid="stSidebar"] { min-width: 230px !important; max-width: 230px !important; background: linear-gradient(180deg, #FFFFFF 0%, #F9FCF6 100%) !important; border-right: 1px solid #E0E7DC !important; }
...
def page_teacher_dashboard():
    ...
    if next_session:
        ...
        <div class="metric-card">
            <h3 style='color: #3D1F5C; margin: 0;'>{next_session['session_num']} - {next_session['lesson']}</h3>
            <p style='color: #666;'>{next_session['date']} ({next_session['day']}) - {next_session['duration']}</p>
            <p>{next_session['content']}</p>
        </div>
        """, unsafe_allow_html=True)
...
def page_session_live():
    ...
    with st.form("session_form"):
        ...
        score = st.slider("Puntuacion (0-100)", 0.0, 100.0, 50.0, 5.0)
...
def page_lessons():
    ...
    if is_teacher:
        ...
        if lang == "fr":
            docs = [("fr/syllabus_fr.pdf", "Programa del curso"), ("fr/methodology_guide_fr.pdf", "Guia pedagogica"), ("fr/progress_tracker_fr.xlsx", "Progress Tracker FR"), ("fr/corriges_fr.pdf", "Corriges")]
        else:
            docs = [("00- Start_here.pdf", "Guia de inicio"), ("01- Syllabus.pdf", "Programa del curso"), ("04- Preambule.pdf", "Preambulo"), ("05- Methodology_Guide.pdf", "Guia pedagogica"), ("02- Answer_Key_19_Lecons_Course_Slides.pdf", "Corriges PPTX"), ("03- Answer_Key_19_Lecons_Additional_Exercices.pdf", "Corriges ejercicios"), ("Progress_Tracker.xlsx", "Progress Tracker")]
        cols = st.columns(2)
