        ...
        with col1:
            ...
            if os.path.exists(pptx):
                with open(pptx, "rb") as f:
                    st.download_button("Descargar PPTX", f,
                        file_name=f"{prefix}_PPT.pptx",
                        use_container_width=True)
            if os.path.exists(notes):
                with open(notes, "rb") as f:
                    st.download_button("Descargar Notas", f,
        ...
        with col2:
            ...
            if os.path.exists(student):
                with open(student, "rb") as f:
                    st.download_button("Descargar Cuaderno", f,
...
def page_sessions():
    ...
    for row in sessions:
        ...
        with st.expander(f"{status} {row['session_num']} - {row['date']} - "
            ...
            if st.button("Abrir", key=f"open_{row['session_num']}",
                          use_container_width=True):
        ...
        with cols[i % 2]:
            if os.path.exists(filename):
                with open(filename, "rb") as f:
                    st.download_button(f"Descargar - {label}", f,
        ...
        with st.expander(f"**{class_name}** - {title_l} ({level})"):
            ...
            if is_teacher:
                ...
                if os.path.exists(student):
                    with open(student, "rb") as f:
                        st.download_button(
                            f"Descargar cuaderno - {class_name}",
                            f,
                            file_name=f"{prefix}{student_suffix}",
                            key=f"student_{prefix}_{lang}",
                            use_container_width=True)
...
def tts_block(items, lang_code="en-US", cols=2):
    ...
    <html><head><style>
        * {{ box-sizing: border-box; }}
        ...
        .tts-btn {{ background: linear-gradient(135deg, #3D1F5C 0%, #5A2F8A 100%);
            color: white; border: none;
            border-radius: 50%; width: 42px; height: 42px;
            cursor: pointer; font-size: 18px; flex-shrink: 0;
            transition: all 0.2s; box-shadow: 0 4px 12px rgba(61, 31, 92, 0.25); }}
        .tts-btn:hover {{ background: linear-gradient(135deg, #FF6B35 0%, #FF8A5B 100%);
...
def page_pronunciation():
    ...
    st.markdown('<div class="sub-header">'
                ...
                unsafe_allow_html=True)
    st.info("Como usar: Haz clic en el boton para escuchar. "
...
def page_exercises():
    ...
    st.markdown('<div class="main-header">Mis Ejercicios</div>',
                unsafe_allow_html=True)
    if lang == "fr":
                ...
                with st.expander(f"{sub['full_name']} - {sub['lesson']} - "
                    ...
                    if sub["answer"].startswith("Audio:"):
                        ...
                        st.markdown(
                            ...
                            unsafe_allow_html=True)
                    fb = st.text_area("Tu retroalimentación", height=150,
                                       key=f"fb_{sub['id']}",
                    ...
                    if st.button("Enviar", key=f"send_{sub['id']}",
                        ...
                        if fb.strip():
                            ...
                            st.warning("Escribe una retroalimentación")
    with tab2:
    ...
    if stats["attended"] >= stats["total"]:
        ...
        if os.path.exists(cert):
            with open(cert, "rb") as f:
                st.download_button("Descargar certificado", f,
